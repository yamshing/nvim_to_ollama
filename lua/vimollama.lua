...
function Vimollama.new()
  local self = setmetatable({}, Vimollama)
  self.value = 0
	self.m_user_input = ""
  return self
end
 
function Vimollama:test()
	 print(self.m_user_input)
	 self:hoge()
end
 
function Vimollama:hoge()
	print("hoge called") 
end
 
function Vimollama:diff_user_input_and_lines(lines)
...
function Vimollama:show_diff_floating_window(diff_lines)
...
function Vimollama:show_reply_to_floating_win(reply)
  reply = reply:gsub("%z", "")
...
function Vimollama:process_chat_api_request(user_text)
  local payload = {
    model = "qwen2.5-coder",
    messages = {
...
function Vimollama:send_selection_to_chat()
